<template>
  <div class="phone">
    ...
    <div class="email">
      ...
      >
      <button
        :class="['captcha',errors.has('phone')?'disabled-btn':'']"
        :disabled="isDisabled"
        @click="send()"
      >{{msg}}</button>
    </div>
    ...
    <button class="bind" @click="bind()">绑定</button>
...
export default {
  ...
  methods: {
    send() {
      ...
      this.axios
        .post("/u/user/binding/phone/" + id, data, {
          headers: {
    ...
    bind() {
      ...
      this.axios
        .put(
          `/u/user/binding/phone/${id}?phone=${this.phone}&captcha=${
            this.captcha
          }`
        )
        .then(response => {
          console.log(response.data);
          if (response.data.code == 0) {
            if (response.data.data.increaseScore == 20) {
              // 显示提示框
              that.flag = true;
              setTimeout(() => {
                that.flag = false;
